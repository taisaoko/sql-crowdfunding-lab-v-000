...
"SELECT projects.title, (SUM(pledges.amount)- projects.funding_goal) AS over_goal FROM projects, pledges GROUP BY projects.title, pledges.project_id HAVING projects.id = pledges.project_id ORDER BY over_goal >= 0 ASC LIMIT 2"
